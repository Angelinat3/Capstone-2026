aku pengen coba model ocr secara local dengan instruksi seperti berikut: PaddleOCR v4 ONNX - Structured Receipt OCR API
...
Memulai FastAPI server di background.
Mengirim data file gambar dari test_images/ ke API endpoint.
Mencetak response JSON yang didapat dari server ke konsol.
Mematikan server background secara otomatis.

akses repository berikut:https://github.com/adithyarhm/capstone-ocr-api dengan github cli untuk integrasi backend dengan model ocr

kalau model masih belum bisa di pakai di aplikasi ini harap laporkan dimana letak permasalahannya, dan jangan memaksa untuk bisa digunakan.